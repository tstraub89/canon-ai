...
- **`PIPELINE_MANAGED_DOCS`** (`docs/architecture.md`, `docs/codebase-map.md`, `docs/decisions.md`, `docs/patterns.md`, `docs/pipeline-orchestrator.md`, `docs/product-context.md`) — auto-committed when the task's `spec.md` `### Affected Files` table lists the doc. **QA-done widening**: once a task's `qa.status === 'done'`, the allow-list expands to the full `PIPELINE_MANAGED_DOCS` set regardless of Affected Files. QA's Docs Freshness sweep (`AGENTS.md` §Docs Freshness) edits protected docs the spec author couldn't have predicted (lessons-learned promotions, codebase-map drift, patterns/pitfalls touched by the task's behavior change); widening at QA-done avoids forcing a manual spec backfill before `--pr`. Tasks that edit a managed doc *before* QA must still list it in Affected Files. Implementation: `commitHumanReviewFiles` in `scripts/run-task/main.ts` and the mirrored union in `verifyBaseDrift` in `scripts/run-task/validation.ts`.
