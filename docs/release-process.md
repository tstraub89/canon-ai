...
7. Creates the tag on the version-bump commit with `git tag` + `git push` (or reuses it if it already exists there; refuses if it exists elsewhere), then creates the GitHub release against that tag with the extracted body. Tag-first because `gh release create --target <sha>` is rejected for `GITHUB_TOKEN` whenever `<sha>` is not the branch head (cli/cli#9514) — the exact shape of a self-heal push.
