...
| Modifying `status.json` shape | State Schema Discipline | `.canon/templates/status.json`, parsers in `scripts/run-task/state.ts`, `scripts/run-task/git.ts`, `scripts/run-task/validation.ts`, `src/task/index.ts` (`VALID_PHASES`, `assertValidPhase()`) |
...
| Lint / TS suppression / `any` | Lint & Type Safety Policy | `scripts/run-task/prompts/templates/implement.md` |
...
**Files**: `.canon/templates/status.json`, parsers in `scripts/run-task/state.ts`, `scripts/run-task/git.ts`, `scripts/run-task/validation.ts`, `src/task/index.ts` (`VALID_PHASES`, `assertValidPhase()`)
...
> Always-applicable rules. *(Canonical home is the `implement` prompt template (`scripts/run-task/prompts/templates/implement.md`); reproduced here for convenience when a contributor is reading patterns mid-task.)*
...
The post-QA window is now closed by the QA-end commit: when `qa.status` becomes `done`, `commitQaArtifacts` commits the task artifact dirs, telemetry, and dirty managed docs before the pipeline stops at `human_review`. That means `--reroute` and `--pr` start from committed post-QA state. The remaining uncommitted-state risk is earlier, before QA has ever completed; per-phase artifact commits would be the broader structural fix for that residual window.
