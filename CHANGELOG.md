...
Hotfix release responding to GP dogfood (starter-preview-renderer bundle) that exposed four related failure classes in the 1.2.0 auto-commit pipeline: the parser silently extracted bare filenames from Codex's combined/wildcard rows; the state machine had no operator escape hatch when manual recovery was needed; gitignored build-generated artifacts in the Changes table triggered the "untracked but git status didn't report" branch (codex even flagged it "report this as a bug"); and adopters running canon in a fresh repo hit a hard codex-CLI failure when the workspace wasn't yet in `~/.codex/config.toml`'s trusted projects.
