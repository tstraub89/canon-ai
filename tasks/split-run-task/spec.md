...
- [ ] **AC-2**: The directory `scripts/run-task/` contains the following code modules, each with the responsibilities described:
    ...
    - `state.ts` — `readStatus`, `writeStatus`, `deriveTopLevelStatus`, `taskDirFor`, `statusFileFor`, `resolveTaskCwd`, session-id storage helpers (`storeSessionId`, `getStoredSessionId`, `toResumePrompt`).
    ...
    - `worktree.ts` — `worktreePath`, `isWorktreeEnabled`, `getActiveCwd`, `findExistingWorktreeForBranch`, `ensureWorktree`, `teardownWorktree`, `flushWorktreeTelemetry`, `syncWorktreeArtifacts`, `syncWorktreeTelemetry`, `TASK_ARTIFACT_FILES`.
    - `validation.ts` — `validateHandoff`, `parseValidationRequiredChecks`, `parseValidationOutcomeRows`, `canonicalizeValidationCheck`, `isPassResult`, `isNAResult`, `escapeRegExp`.
...
- [ ] **AC-5**: Phase prompts live under `scripts/run-task/prompts/`:
    ...
    - `helpers.ts` — `taskList`, `phaseCommands`, `CLAUDE_STARTUP`, `CODEX_STARTUP`, `QA_STARTUP`.
