...
- **`canon task accept <id...> <phase> [--force]`** — operator escape hatch when work has been manually committed outside the pipeline and `canon run` keeps re-running auto-commit against the already-landed commit. Marks the phase done and sets `phases.<phase>.operator_accepted: true` so the next dispatch skips auto-commit. Accepts multiple task IDs for bundle mode — the handoff coverage check unions every task's handoff against one `baseRef..HEAD` diff, so siblings don't cross-reject. Currently only the `implement` phase is supported. Guards: clean source-tree, non-empty `baseRef..HEAD`, no malformed handoff rows, handoff covers the diff; `--force` bypasses all four. Acceptance is pinned to `operator_accepted_sha` (HEAD at accept time) and auto-cleared whenever `implement` moves away from `done` (reroute, `changes_requested`, manual `canon task phase`). Auto-commit re-checks working-tree cleanliness on each dispatch so an accept followed by uncommitted edits cannot silently bypass it.
...
- **Strict handoff `## Changes` table parser.** The prior `` /`([^`]+)`/ `` regex returned the first backticked token in column 1, so combined rows like `` `a.ts`, `b.ts` ``, prose with embedded paths, and unfilled `<path>` template placeholders all silently produced wrong (or no) paths. The new parser anchors each cell to either `` `path` `` or `[path](url)`, rejects multiple paths per cell, rejects `*` and `?` wildcards (square brackets stay valid — they're real filename characters), rejects unfilled angle-bracket placeholders, and rejects absolute paths or `..`-traversals. Errors surface at preflight, auto-commit, and evidence-advance with the specific rejection reason per cell instead of failing later with cryptic "missing file" diagnostics.
- **Handoff template no longer ships a literal `` | `<path>` | ... | `` example row.** Both the baseline `## Changes` table and the iteration `### Changes` block start with an empty body and a one-line format hint above. The strict parser would have rejected the placeholder anyway; removing it from the template avoids the rejection in the first place.
