...
Hotfix release responding to GP dogfood (starter-preview-renderer bundle + a separate first-canon-task report) that exposed five related failure classes in the 1.2.0 auto-commit pipeline plus three adopter-experience issues: the parser silently extracted bare filenames from Codex's combined/wildcard rows; the state machine had no operator escape hatch when manual recovery was needed; gitignored build-generated artifacts in the Changes table triggered the "untracked but git status didn't report" branch (codex even flagged it "report this as a bug"); adopters running canon in a fresh repo hit a hard codex-CLI failure when the workspace wasn't yet in `~/.codex/config.toml`'s trusted projects; `canon task list` crashed if any task directory had a non-canonical `status.json`; bundled `canon task accept` cross-rejected sibling tasks' files; and the `--pr` flow advertised canon in every PR body.

### Added

- **`canon task accept <id...> <phase> [--force]` — operator escape hatch for manually-committed work.** When an operator has manually committed implementation work on the task branch and `canon run` keeps re-running auto-commit against the already-landed commit, `canon task accept` marks the phase done AND sets `phases.<phase>.operator_accepted: true` so the post-phase dispatch (auto-commit for implement) is skipped on subsequent runs. Today only the `implement` phase is supported. **Accepts multiple task IDs for bundle mode** — the handoff coverage check unions every task's handoff against one `baseRef..HEAD` diff, so siblings don't cross-reject each other's files. All tasks in a bundle must share `base_branch` and working tree. Guards: (1) clean source-tree (pipeline-owned paths like `tasks/<id>/...` and `PIPELINE_TELEMETRY_FILES` exempt), (2) non-empty `baseRef..HEAD`, (3) handoff Changes table has no malformed rows, (4) handoff coverage matches the diff (every diff file in some task's handoff and vice versa). `--force` bypasses all four. The acceptance is pinned to `operator_accepted_sha` (HEAD at accept time) — a later commit on the task branch invalidates the skip — and is automatically cleared whenever `implement` is moved away from `done` (reroute, changes_requested, manual `canon task phase`). Auto-commit also re-checks working-tree cleanliness on each dispatch so an accept followed by uncommitted edits cannot silently bypass auto-commit. Surfaced by the GP starter-preview-renderer wedge.
- **`canon doctor` checks codex project trust.** New `checkCodexProjectTrust` reads `~/.codex/config.toml`, parses the `[projects."<path>"]` blocks, and warns if the current workspace (git toplevel) isn't trusted — including the exact one-line TOML block to add. Handles inline `#` comments on both the table header and the `trust_level` line, applies nearest-ancestor inheritance (so `/Users/x` trusted covers `/Users/x/repo` if there's no override), and treats an explicit non-trusted child entry as overriding a trusted parent. Without this, `codex exec` from a fresh adopter checkout fails hard before canon ever sees output — surfaced by the GP first-run experience.
...
- **Gitignored handoff entries are exempt from existence and coverage checks.** Build-generated artifacts like `public/sitemap.xml` that Codex legitimately references in the Changes table to describe build output cannot appear in `git diff base...HEAD` (not tracked), `git status` (ignored), or `ls-files --error-unmatch` (untracked) — so 1.2.0 hit the "untracked on disk but git status did not report it (report this as a bug)" branch on every run. New `filterGitIgnoredPaths(paths, cwd)` helper runs a single batched `git check-ignore --stdin -z` and the returned set is used to skip those entries in: `autoCommitCode`'s missing-file precheck, `verifyHandoffFilesCommitted`'s post-commit verification, `verifyHandoffAgainstDiffFromData`'s handoff→diff check (via new optional `gitIgnoredHandoffFiles` input), `tryEvidenceAdvance('implement')`'s on-disk check, and `taskAccept`'s coverage check. Evidence-advance still refuses to advance when ALL listed files are gitignored (no real source evidence). Gitignore lookup uses the active worktree's cwd, not the supervising checkout, so branch-local `.gitignore` rules are respected. Surfaced by the GP starter-preview-renderer wedge.
- **`canon task list` no longer crashes on non-canonical `status.json`.** Adopter repos accumulate manual, imported, or older task files whose `status.json` may not have the canonical `phases.spec` shape. The 1.2.0 listing called `derivePhase()` (which reads `status.phases[phase]?.status`) without guarding for a missing `phases` object — so one malformed row crashed the entire listing with `Cannot read properties of undefined (reading 'spec')`, hiding every valid task. Each row is now wrapped: invalid `status.json` files render as `INVALID: <reason>` in the `CURRENT PHASE` column, the listing continues, and `canon task list` exits non-zero if any invalid was encountered. Filed by James in [#83](https://github.com/tstraub89/canon-ai/issues/83); regression test mirrors his repro.
- **`canon run --pr` no longer leaks "Auto-generated by canon-ai" into PR bodies.** The 1.2.x flow unconditionally passed `--body "Auto-generated by canon-ai for <label>."` to `gh pr create`, advertising the tool in every adopter PR. Default behavior now omits `--body` entirely so `gh` falls back to its own defaults (`.github/pull_request_template.md` if present, else the commit messages). Adopters who want canon attribution can opt in via the new `CANON_PR_BODY` env var, which supports `$LABEL` (task ID, or comma-joined IDs for bundles) and `$TITLE` (PR title) placeholders. Surfaced by GP "ninja mode" first-canon-task report.
- **`canon run --pr` at `human_review` is now truly idempotent on the dirty-tree path.** The 1.2.0 changelog claimed `--pr` was idempotent when a PR already exists ([issue #72](https://github.com/tstraub89/canon-ai/issues/72)), but only the clean-tree retry branch actually had the existing-PR check — the dirty-tree commit-then-create path went straight to `gh pr create` and died on its "PR already exists" exit code. GP's starter-preview-renderer bundle hit this when QA finished writing `tasks/<id>/done.md` + telemetry on an already-PR'd branch: pipeline completed cleanly, but `--pr` exited 1 reporting `gh pr create` failure. Both paths now route through a new `reportOrCreatePR` helper that checks for an open PR on the (branch, base) pair via the gh-authoritative `findOpenPRNumber` lookup before calling `createDraftPRForTask`. If found, prints the URL and returns 0; otherwise creates as before. Regression test covers the dirty-tree path matching GP's repro.
