...
Solution: expand `commitTaskArtifactsToBase` at [git.ts:83](../../scripts/run-task/git.ts:83) to also stage and commit dirty `PIPELINE_TELEMETRY_FILES` at scaffold-commit time. The scaffold absorbs pre-implement telemetry unconditionally. REPO_ROOT is clean after `runImplementPhase` starts.

The expanded body has two strict phases (per AC-22d): (1) **per-task scaffold commits** — path-restricted with `git commit --only -- <taskDir>` so they don't sweep pre-staged telemetry; (2) **telemetry absorption** — one final commit with a distinct message (`chore: absorb pre-implement telemetry into scaffold for <taskId-list>`). The current `commitTaskArtifactsToBase` at [git.ts:90](../../scripts/run-task/git.ts:90) uses a bare `git commit -m '...'` (no pathspec, no `--only`) which would commit everything in the index — that's a footgun the AC-22d rewrite closes. This codifies the discipline operator Claude already follows manually (commit clean state before spawning the worktree).

**Cross-task telemetry contamination is out of scope for this task.** A pre-flight gate that discriminates current-task rows from foreign-task rows (the original AC-22g) was carved out after 5 spec_review iterations each found new bugs in the parser-based design (per-file parsers for three differently-formatted files, bundle-label splitting, both-staged-and-unstaged diff inspection, append-only discipline). The successor design uses byte-offset snapshotting (telemetry is `appendFileSync`-only by code path; snapshot file sizes at orchestrator entry, anything before the snapshot is foreign) and is filed as a separate BACKLOG entry — see *Telemetry discrimination gate at scaffold-commit time*. **Interim mitigation**: operator discipline. Before `canon run <id>` on a new task, ensure `git status --porcelain -- docs/pipeline-invocations.md docs/task-quality-log.md docs/lessons-learned.md` is clean — any dirty rows from a prior task get absorbed into the new task's scaffold commit. Misattribution is detectable in PR review (the rows contain task IDs in their content) and recoverable (`git revert` + re-commit under the right task).
...
- [ ] AC-2: `taskDirFor(taskId)` at [state.ts:34](../../scripts/run-task/state.ts:34) is rewired. New body must preserve `CANON_TASKS_DIR_OVERRIDE` precedence:
  ...
  Function signature unchanged. Verify by reading the source: override branch precedes `resolveTaskCwd`; non-override branch routes through `resolveTaskCwd`.
...
- [ ] AC-15: All other tests pass. The full `npm test` suite reports zero failures after the AC-14 deletions. Other tests that may have implicitly depended on the sync (e.g., reading REPO_ROOT post-pipeline expecting synced content) are updated to either (a) write directly to the side being tested, or (b) read from the worktree path explicitly. Audit during implementation; update each. Note on `CANON_TASKS_DIR_OVERRIDE`: tests that set this override continue to work because **`taskDirFor` honors the override directly via its own fast-path branch (AC-2)** — the override no longer requires propagation through `resolveTaskCwd`. This is important because `resolveTaskCwd` checks `dev-worktrees/<id>/tasks/<id>/status.json` first ([state.ts:42-43](../../scripts/run-task/state.ts:42)); if a test set both the override AND constructed a fake worktree directory at the conventional path, routing through `resolveTaskCwd` could pick the worktree over the override. AC-2's override fast-path prevents that.
...
- [ ] AC-21d: `canon task list` reads from worktrees for tasks that have them. Note: `taskList()` at [src/task/index.ts:247-297](../../src/task/index.ts:247) does NOT call `taskDirForCwd` today — it walks `fs.readdirSync(tasksRoot())` and reads `path.join(root, entry, 'status.json')` directly, so the AC-21b rewire does NOT propagate to list automatically. Explicit rewrite required: replace the per-entry `path.join(root, entry, 'status.json')` at [src/task/index.ts:260](../../src/task/index.ts:260) with `path.join(taskDirForCwd(process.cwd(), entry), 'status.json')`. The directory enumeration still walks `tasksRoot()` (REPO_ROOT-relative — the canonical "what tasks exist" list lives there because every task starts pre-implement at REPO_ROOT); only the status-file read switches to the worktree-aware helper. Verify with an integration test: create two tasks, one worktree-mode (with a populated worktree containing a status.json that differs from REPO_ROOT's scaffold) and one not, run `canon task list`, assert the worktree-mode row reflects the worktree's status.json content (e.g., its current phase) and the REPO_ROOT-only row reflects its REPO_ROOT scaffold.
...
- [ ] AC-22b: The `activeCwd` is plumbed all the way through to `getMetricsFile()` via the `MetricEntry` shape, **not** by changing `getMetricsFile()` call sites directly (those don't exist in the agents — they sit one layer up). Three coordinated changes:
  1. **Type extension** — `MetricEntry` in [types.ts:142](../../scripts/run-task/types.ts:142) gains `activeCwd?: string` (optional for backward-compat with any synthetic test callers).
  2. **Producer extension** — the `metricsContext` parameter type in [agents/claude.ts:63](../../scripts/run-task/agents/claude.ts:63) and [agents/codex.ts:15](../../scripts/run-task/agents/codex.ts:15) gains `activeCwd?: string`. The `recordMetric({ ...metricsContext, ... })` spread at [agents/claude.ts:225](../../scripts/run-task/agents/claude.ts:225) and [agents/codex.ts:104](../../scripts/run-task/agents/codex.ts:104) then automatically forwards it onto the `MetricEntry`.
  3. **Consumer extension** — `recordMetric` in [metrics.ts:13-14](../../scripts/run-task/metrics.ts:13) changes `const metricsFile = getMetricsFile();` to `const metricsFile = getMetricsFile(entry.activeCwd);`.
  4. **Caller updates** — each phase module that builds `metricsContext` adds `activeCwd: getActiveCwd(taskIds)`. Audit during implementation: at minimum `phases/spec.ts`, `phases/spec-review.ts`, `phases/plan.ts`, `phases/implement.ts`, `phases/code-review.ts`, `phases/qa.ts` (the seven phase modules that invoke an agent). Pre-implement phases (`spec`, `spec_review`, `plan`) naturally supply `REPO_ROOT` because `getActiveCwd` returns REPO_ROOT when no worktree exists; post-implement phases supply the worktree path. No phase module needs special-casing — `getActiveCwd(taskIds)` is the single source.

  Verify by reading the source: `MetricEntry` has `activeCwd?: string`; both agents' `metricsContext` type signatures include `activeCwd?: string`; `recordMetric` reads `entry.activeCwd`; every phase module that builds `metricsContext` includes `activeCwd: getActiveCwd(taskIds)`.
...
- [ ] AC-22d: `commitTaskArtifactsToBase` at [git.ts:83-93](../../scripts/run-task/git.ts:83) is expanded to also stage and commit dirty `PIPELINE_TELEMETRY_FILES` at scaffold-commit time. The function gains a strict two-phase shape:

  1. **Per-task scaffold phase**: iterate `taskIds`. For each task with dirty `taskDir` content (per `git status --porcelain -- <taskDir>`), `git add -- <taskDir>` then commit with **path-restricted** semantics: `git commit -m '...' --only -- <taskDir>`. The `--only` flag prevents the commit from sweeping in any pre-staged telemetry (or other pre-staged content) — without it, a plain `git commit` would include everything in the index, defeating AC-22d's "telemetry gets its own commit" invariant. Verify by reading the source: every per-task scaffold commit uses `--only --` with the taskDir pathspec; no bare `git commit` calls remain in this function.
  2. **Telemetry-absorption phase**: iterate `PIPELINE_TELEMETRY_FILES`, check each with `git status --porcelain -- <file>`, stage dirty ones via `git add -- <file>`, and produce ONE additional commit (`git commit -m 'chore: absorb pre-implement telemetry into scaffold for <taskId-list>' --only -- <file...>`) covering all dirty telemetry with a message distinct from the per-task scaffold message. `--only` here is defensive — it ensures the commit only contains the listed telemetry files, even if something else slipped into the index. If no telemetry is dirty, this phase is a no-op. Absorption is **unconditional** — no foreign-vs-current-task discrimination (carved out to a separate BACKLOG entry; see Change 5). Operator discipline is the interim mitigation: commit/discard prior-task telemetry before invoking `canon run` on a new task.

  Do NOT fold telemetry into the per-task commits — that would either double-stage in bundle mode or arbitrarily attribute telemetry to the first task. Verify by reading the source: per-task commits are path-restricted with `--only`; telemetry staging+commit happens once at the end; the telemetry commit message is distinct.
...
| `scripts/run-task/main.ts` | (1) Remove the post-phase sync calls at lines 2404-2405. (2) Update the archive-move at line 1692 from `taskDirFor(taskId)` to `taskDirForRepoRoot(taskId)` (import from state.ts). (3) The re-export `const taskDirFor = splitState.taskDirFor` at line 98 stays — propagates the rewired function. (4) Other `taskDirFor` consumers (line 182, 1984, 2012, 2013, 2083) are unchanged — naturally inherit the rewire. (5) DELETE `mirrorHumanReviewDocsToCwd` (lines 647-678 approx). (6) Remove the call to `mirrorHumanReviewDocsToCwd(cwd)` at line 905 in `commitHumanReviewFiles`. (Metrics threading does NOT change main.ts call sites — it flows through `MetricEntry.activeCwd`; per-phase modules supply `activeCwd: getActiveCwd(taskIds)` in `metricsContext` directly.) |
| `scripts/run-task/git.ts` | (1) Update `commitTaskArtifactsToBase` at line 86 from `taskDirFor(taskId)` to `taskDirForRepoRoot(taskId)`. Add import for `taskDirForRepoRoot` from `state.ts`. (2) Restructure `commitTaskArtifactsToBase` body into two strict phases per AC-22d: **per-task scaffold commits** with `git commit -m '...' --only -- <taskDir>` (the existing bare `git commit -m '...'` at line 90 is replaced — without `--only` the commit sweeps in any pre-staged telemetry, which would defeat the "telemetry gets its own commit" invariant); **telemetry-absorption commit last** with `git commit -m 'chore: absorb...' --only -- <file...>` (unconditional — discrimination is deferred to the BACKLOG follow-up). (3) Import `PIPELINE_TELEMETRY_FILES` from `worktree.ts`. Signature unchanged from current `(taskIds, _artifactFiles)`. |
| `scripts/run-task/phases/implement.ts` | Delete the REPO_ROOT → worktree copy loop at lines 48-64. The `commitTaskArtifactsToBase` call at line 44 is unchanged (no signature changes propagate from this task). |
| `scripts/run-task/metrics.ts` | (1) Update `getMetricsFile()` at line 7 to accept an optional `activeCwd?: string` parameter. New body: `return process.env.CANON_METRICS_FILE_OVERRIDE ? path.resolve(...) : path.join(activeCwd ?? REPO_ROOT, 'docs/pipeline-invocations.md');`. (2) Update `recordMetric` at line 13-14 — change `const metricsFile = getMetricsFile();` to `const metricsFile = getMetricsFile(entry.activeCwd);` so the `MetricEntry`'s `activeCwd` field reaches the file resolver. |
| `scripts/run-task/types.ts` | Add `activeCwd?: string` to `MetricEntry` at line 142. Optional for backward-compat. |
| `scripts/run-task/agents/claude.ts` and `scripts/run-task/agents/codex.ts` | Extend the `metricsContext` parameter type to include `activeCwd?: string` ([claude.ts:63](../../scripts/run-task/agents/claude.ts:63), [codex.ts:15](../../scripts/run-task/agents/codex.ts:15)). The existing `recordMetric({ ...metricsContext, ... })` spread at claude.ts:225 / codex.ts:104 automatically forwards `activeCwd` to the `MetricEntry`. No call-site changes inside the agents. |
| `scripts/run-task/phases/spec.ts`, `phases/spec-review.ts`, `phases/plan.ts`, `phases/implement.ts`, `phases/code-review.ts`, `phases/qa.ts` | Every `metricsContext` build site adds `activeCwd: getActiveCwd(taskIds)`. Pre-implement phases get REPO_ROOT (no worktree yet); post-implement get the worktree. Audit during implementation — exact line numbers per phase module. |
| `src/task/index.ts` | (1) Update `taskDirForCwd(cwd, taskId)` at line 65-70 to apply worktree-resolution. Import `resolveTaskCwd` from `../../scripts/run-task/state.js` (relative path resolves through tsup bundle). Body: when `tasksRoot()` returns an absolute path (override set), return `path.join(root, taskId)` (override wins). Otherwise call `resolveTaskCwd(taskId)`; if it returns a worktree path (not REPO_ROOT-equivalent), return `path.join(<worktreePath>, 'tasks', taskId)`; else return the previous `path.join(cwd, root, taskId)`. Signature unchanged. Callers `taskStatus`/`taskAccept`/`taskPhase` at lines 377, 471, 733, 761 inherit automatically. (2) Update `taskList()` at line 247-297 — the `path.join(root, entry, 'status.json')` read at line 260 switches to `path.join(taskDirForCwd(process.cwd(), entry), 'status.json')` so list reads worktree state per-entry. Directory enumeration still walks `tasksRoot()` (REPO_ROOT-relative — every task starts at REPO_ROOT pre-implement). |
...
| `CLAUDE.md` | Update "Reroute" section convention per AC-18. Add a one-liner to "Quick refs" reflecting the **new** CLI behavior under this task: `canon task status <id>` (and `list`/`accept`/`phase`) run from REPO_ROOT now read the worktree's status.json when one exists past plan — mid-pipeline status reads show live worktree state, not the frozen scaffold. This is the parser-cwd-style fix applied to the operator CLI. Do NOT ship the inverse guidance (telling operators to `cd dev-worktrees/<id>` for live state); that describes the pre-fix behavior and would mislead. |
...
| `docs/architecture.md` | Update Tech Stack Worktree section per AC-22h. |
...
| `tests/run-task-validation.test.ts` | Update any tests that depended on the now-deleted sync (audit during implementation). Tests using `CANON_TASKS_DIR_OVERRIDE` continue to work — override propagation has TWO paths: (a) `taskDirFor`'s own fast-path branch (AC-2) returns the override directly without consulting `resolveTaskCwd`, and (b) `taskDirForRepoRoot` honors the override too. |
...
- **`mirrorHumanReviewDocsToCwd`** at [main.ts:642](../../scripts/run-task/main.ts:642) — DELETED (per AC-22c). Under SSOT past plan, the worktree's telemetry is already canonical; copying REPO_ROOT's (necessarily partial) telemetry over the worktree's would overwrite valid content. The call site at [main.ts:905](../../scripts/run-task/main.ts:905) in `commitHumanReviewFiles` is removed in the same diff.
...
- [x] `build` (`npm run build`) — required per `docs/architecture.md` Full build binding; CI gates on `git diff --exit-code -- dist/`
- [ ] `E2E` — N/A; no UI
...
- **Cross-task telemetry contamination at scaffold-commit time** — AC-22d's absorption commit is unconditional under this task; it commits whatever is dirty in `PIPELINE_TELEMETRY_FILES` regardless of which task's rows are present. If the operator runs `canon run B` while `pipeline-invocations.md` still has uncommitted rows from a prior `canon run A`, Task A's rows land under Task B's scaffold commit message. **Interim mitigation**: operator discipline — before `canon run <id>` on a new task, ensure `git status --porcelain -- docs/pipeline-invocations.md docs/task-quality-log.md docs/lessons-learned.md` is clean. Misattribution is detectable in PR review (the rows contain task IDs in their content) and recoverable (`git revert` + re-commit). **Structural fix**: filed as a separate BACKLOG entry (*Telemetry discrimination gate at scaffold-commit time*) using byte-offset snapshotting instead of the parser-based approach this task originally proposed. Carved out after 5 spec_review iterations established that the parser surface is intrinsically too complex for the value it provides; the snapshot-offset successor design is format-agnostic and ~110 lines total.
...
- **No REPO_ROOT → worktree mirror of telemetry post-plan**. With `mirrorHumanReviewDocsToCwd` deleted (AC-22c), the worktree's telemetry is canonical from implement onward. Project-level managed docs (`docs/codebase-map.md`, `docs/decisions.md`, `docs/patterns.md`, `docs/architecture.md`, `docs/product-context.md`, `docs/pipeline-orchestrator.md`) are out of scope here — their cross-worktree coordination is the separate sync-rewrite BACKLOG entry. Be precise in `docs/decisions.md`: "no telemetry mirror after plan; managed-doc coordination is a separate concern."
- **`src/task/index.ts` CLI surface alignment**. Under this task, CLI commands (`canon task status / list / accept / phase`) become worktree-aware alongside pipeline-internal callers — both surfaces read worktree state when one exists past plan. The exception is `canon task new`, which always writes to REPO_ROOT (pre-implement, no worktree yet — `resolveTaskCwd` returns REPO_ROOT and the rewire is a no-op). Future maintainers: do NOT reintroduce a REPO_ROOT-only path for the operator-facing list/status reads — that mismatch (pipeline writes worktree, CLI reads REPO_ROOT) is the parser-cwd bug class.
