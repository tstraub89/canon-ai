...
| Unit tests | `npm test` (= `node --test --import ./tests/md-loader-register.mjs --import tsx tests/*.test.ts`). The glob is hard-coded in the script, so `npm test -- <file>` does **not** scope the run — the extra path is appended and the full suite still runs. To scope, invoke the runner directly with the same imports: `node --test --import ./tests/md-loader-register.mjs --import tsx tests/cli.test.ts`. Report scoped runs in a Validation table only when they were actually scoped. |
