...
**Rule**: New task-facing provenance should read from `status.json.canon`. The canonical upstream repo slug lives in `CANON_UPSTREAM_REPO` in `src/orchestrator/canon-snapshot.ts`; do not duplicate that value in docs or handoff artifacts unless you're explicitly pointing back to the symbol. `captureCanonSnapshot()` may override the stamped slug at call time from a non-empty `CANON_UPSTREAM_REPO` env var, but the symbol remains the default source of truth.
...
**Decision**: Tier detection, sizing, model/effort selection, and loop-cap defaults live in a pure side-effect-free module (`src/lib/pipeline-policy.ts`). The orchestrator passes resolved config in; the policy returns decisions out.
...
**Rule**: Any new routing decision (model choice, effort, tier, loop cap) goes in `pipeline-policy.ts`. Add a row to `pipeline-policy.test.ts`. Do not write inline routing in `src/orchestrator/main.ts` or the phase modules.
...
**Decision**: Canon-managed and shipped guidance must not point adopters at canon orchestration internals — the orchestrator source under `src/orchestrator/**`, canon's CLI source elsewhere under `src/**`, or the per-phase prompt templates within the orchestrator subtree. Adopters customize task templates by copying them into `tasks/_templates/` (which `canon task new` prefers over the canon-shipped `.canon/templates/` defaults, and which `canon upgrade` never overwrites); canon's shipped surfaces otherwise stay decoupled from the implementation machinery adopters do not have. This is the broad principle; enforcement of it is split between a mechanical gate and authoring/review discipline (see Rule).

**Why**: Canon ships into adopter repos through upgradeable guidance surfaces, while the orchestration internals live only in canon's own checkout. If shipped prose points at those internals, adopters get broken references — surfaced when their `docs-refs-check` next runs (canon invokes it at `--pr`/commit time in `src/orchestrator/main.ts`, and in CI for repos that wire it up), not at `canon upgrade` itself — or instructions they cannot follow. Keeping shipped guidance on adopter-facing phases, concepts, and overridable templates preserves the contract without leaking canon's implementation details. The v2.0.0 leak (`adopter-agent-file-redesign`, a shipped skill naming the internal `implement.md` prompt template) is the canonical instance this rule guards against.

**Rule**:

- **The leak gate in `scripts/sync-canon-templates.mjs` mechanically enforces the unambiguous subset** of this principle, and the gate (run via `npm run sync-templates:check`) must pass before shipping canon-managed markdown, skills, or templates. The gate discriminates by path specificity and blocks two reference classes because they are *unambiguously* canon-internal: (1) path refs under the specific two-segment subtree `src/orchestrator/` (including its prompt templates), and (2) bare basenames of internal-only per-phase prompt templates — those under `src/orchestrator/prompts/templates/` that have no `.canon/templates/` or task-artifact counterpart (e.g. `qa.md`, `implement.md`). It deliberately does **not** blanket-block the bare `src/` or `scripts/` roots: those names are ambiguous — adopters have their own source and tooling roots, and shipped guidance legitimately references them as concepts. The specific `src/orchestrator/` prefix is unambiguous in exactly the way the orchestrator's former two-segment subtree was; a blanket top-level-prefix block would false-positive on correct adopter-facing prose.
...
**Why**: The alternative — shipping one concrete model as "the" canon workflow — produced recurring scope creep: prescriptive release-branch language crept back into adopter-facing surfaces multiple times because nothing pinned the stance. The orchestrator has been model-agnostic in code (`getBaseBranch()` in `src/orchestrator/git.ts` reads `base_branch` from `status.json` with no hardcoded `dev`/`release/` assumption) since before v1.0.0; the guidance lagged. Recording the stance as a settled decision is the anti-regression guard.
...
**Decision**: After Codex's `implement` phase passes validation, the orchestrator (`autoCommitCode()` in `src/orchestrator/main.ts`) parses the handoff Changes table and creates the implement commit. Codex does not run `git commit` itself.
...
## Track work in GitHub Issues by shape; keep BACKLOG.md for design-of-record

**Decision** (revised 2026-09-05 at the open-source launch; supersedes the 2026-05 BACKLOG-first rule below): new work goes to GitHub Issues on `tstraub89/canon-ai` when it is *issue-shaped* — a bug with a reproduction, a bounded feature with a clear "done", or anything an outside contributor might pick up or follow. `docs/BACKLOG.md` is reserved for *design-of-record*: long-form entries with sequencing dependencies, unresolved design tensions, or cross-references to other entries, the kind a spec-writing session needs cold-loaded from the repo. An entry that has both a design record and a trackable outcome carries a one-line link to its issue so the two do not drift.

**Why**: the original rule was written for a private repo with one developer and no external filers, and it named its own reversal condition ("if canon goes OSS later"). Public visibility changes the trade: issues are how contributors see what is open and how the maintainer signals "wanted" without committing to a design. What the original rule got right still holds — issue bodies compress prose, and pipeline agents read the repo, not GitHub, so design that a spec needs must live in `docs/`. Hence the split by shape rather than a wholesale move.

**Rule**: when framing new work, ask which shape it is. Issue-shaped → open an issue (labels `bug` / `enhancement`), and if a BACKLOG entry already holds the design, link the two. Design-shaped → a BACKLOG entry, with an issue only once there is a concrete deliverable to track. Adopter-filed issues stay as issues regardless. Do not duplicate a full design into an issue body; point at the entry.

**Original (2026-05) rule, retained for history**: New work items captured during canon-ai development land in `docs/BACKLOG.md`, not as GitHub issues. GitHub issues are reserved for three specific cases: (1) bugs and feature requests filed by external adopters — the filer's choice of surface, leave them where they are; (2) items about to land in a PR that wants `Closes #N` auto-close; (3) items with active asynchronous discussion involving someone outside the current session. Rationale at the time: BACKLOG's prose-and-cross-reference format suited design-stage entries, and at single-developer private-repo scale GitHub's issue affordances went unused.
