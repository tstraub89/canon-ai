...
- `--ship` ENOENT on non-worktree tasks (1.4.x known limitation)
...
Symptom: For a task with `worktree: false`, `canon run <id> --ship` crashes with `ENOENT: no such file or directory, open '<repo>/tasks/<id>/status.json'`. `shipTasks` switches to base before re-reading `status.json`.
...
The post-merge `--ship` works because the task dir is now on the base branch via the squash. Worktree-mode tasks are unaffected.
