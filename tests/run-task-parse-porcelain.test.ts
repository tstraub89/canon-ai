...
test('parsePorcelain expands new directories when fed -uall output', () => {
...
test('parsePorcelain without -uall collapses new directories — the wall-textures regression', () => {
...
test('parsePorcelain handles renames by emitting both old and new paths', () => {
...
test('parsePorcelain strips surrounding quotes on paths with spaces', () => {
...
test('parsePorcelain returns empty set for empty input', () => {
    assert.equal(parsePorcelain('').size, 0);
    assert.equal(parsePorcelain('   \n').size, 0);
});

test('parsePorcelain rejects trimmed porcelain that loses the leading worktree column', () => {
    assert.throws(
...
test('findUncoveredTrackedChanges catches staged-only source changes outside handoff', () => {
    const status =
        'M  src/unrelated.ts\n' +
        ' M api/claim-pro.ts\n' +
        '?? asdf\n' +
...
test('findUncoveredTrackedChanges requires both sides of a rename to be in handoff', () => {
...
test('findStagedFilesOutsideHandoff catches files git commit would otherwise sweep in', () => {
    const staged =
        'api/claim-pro.ts\n' +
        'src/unrelated.ts\n' +
...
test('PIPELINE_TELEMETRY_FILES covers the two files the pipeline itself writes', () => {
...
test('isDoneMdTemplate returns true when done.md is missing', () => {
    const p = path.join(os.tmpdir(), `run-task-test-missing-${Date.now()}.md`);
    assert.equal(isDoneMdTemplate(p), true);
});

test('isDoneMdTemplate detects unfilled template via [TASK-ID] sentinel', () => {
    withTempFile('# Completion Summary: [TASK-ID] — [Title]\n\n## What Changed\n\nOne paragraph, plain English. No code jargon.\n', p => {
        assert.equal(isDoneMdTemplate(p), true);
    });
});

test('isDoneMdTemplate detects unfilled template via "src/..." sentinel', () => {
    withTempFile('# Completion Summary: my-task\n\n## Files Changed\n- `src/...` — brief note\n', p => {
        assert.equal(isDoneMdTemplate(p), true);
    });
});

test('isDoneMdTemplate returns false for a real QA summary', () => {
    const real =
...
test('extractDoneMdFromStdout accepts a "QA Summary" heading', () => {
...
test('extractDoneMdFromStdout accepts a "Completion Summary" heading', () => {
    const stdout = '# Completion Summary: my-task\n\n## What Changed\n\nBody.\n';
    const out = extractDoneMdFromStdout(stdout);
    assert.ok(out.startsWith('# Completion Summary: my-task'));
});

test('extractDoneMdFromStdout rejects stdout without a recognizable heading', () => {
...
test('extractDoneMdFromStdout returns empty for empty stdout', () => {
    assert.equal(extractDoneMdFromStdout(''), '');
    assert.equal(extractDoneMdFromStdout('   \n  \n'), '');
});

test('extractDoneMdFromStdout trims leading whitespace before the heading', () => {
    const stdout = '\n\n# QA Summary: x\n\nBody.\n';
    const out = extractDoneMdFromStdout(stdout);
