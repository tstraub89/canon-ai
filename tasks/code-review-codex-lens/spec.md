...
- **No edit to historical / point-in-time records** — `CHANGELOG.md`, `docs/lessons-learned.md`, `docs/BACKLOG.md`, `docs/task-quality-log.md` (the dated QA trend log — this task's own row quotes the rule it overturns), and the dated reports `docs/harness-audit-2026-06.md` + `docs/canon-opus48-gpt55-report.md` carry the old "two lenses / do not add a third lens" framing as accurate past record; excluded from the structural sweep (AC-10).
...
- [ ] AC-2: **If the Codex review can't be obtained, `code_review` stops for human re-run — it does not proceed on the two Claude lenses, does not approve, and does not advance to `qa`.** The stop happens before the foreman runs and is re-runnable (a later `canon run` retries the review when Codex is available) — matching how a failed Codex call already halts `implement`/`spec_review`. "Can't be obtained" means the review produced no complete findings output — no `agent_message`, **or a stream that crashed/truncated before completing (no `turn.completed` event)** — **not** a review that ran to completion and merely reported issues. A review that completes is a success whose findings flow to the foreman, gated on the stream-completion event rather than the exit code (so a complete review followed by a benign non-zero teardown still counts as obtained, and a partial `agent_message` from a crashed run does not). (There is no graceful 2-lens fallback — see Non-Goals; and no new verdict — see Non-Goals.) Verify: a phase test where the review can't be obtained asserts the phase stops before the foreman with no advance to `qa`.
...
| `templates/.canon/templates/review.md` | Auto-synced mirror of edited `CANON_OWNED` root — do not hand-edit; listed for the `--pr` base-drift allowlist (regenerated by `npm run sync-templates`). |
| `templates/.claude/agents/code-review-cold.md` | Auto-synced mirror of edited `CANON_OWNED` root — do not hand-edit; listed for the `--pr` base-drift allowlist (regenerated by `npm run sync-templates`). |
| `templates/.claude/agents/code-review-anchored.md` | Auto-synced mirror of edited `CANON_OWNED` root — do not hand-edit; listed for the `--pr` base-drift allowlist (regenerated by `npm run sync-templates`). |
| `templates/docs/pipeline-orchestrator.md` | Auto-synced mirror of edited `CANON_OWNED` root — do not hand-edit; listed for the `--pr` base-drift allowlist (regenerated by `npm run sync-templates`). |
