...
- **Claude `code_review` upgrades to Opus on L/XL/delicate.** Previously the entire Claude code-review matrix ran on `CLAUDE_MODEL_REVIEW` (default Sonnet) regardless of size — delicate/XL only bumped effort to xhigh. That left a Codex/Claude tier asymmetry: Codex `implement` on XL/delicate ran `codexModelFull` while Claude review of the same code ran Sonnet. Observed effect: post-PR `codex review` was catching lifecycle/state-machine bugs (buffer-arming-on-failure, project-switch flushes, stale-runId guards) that Sonnet code_review was missing. The matrix now splits — S/M keep `CLAUDE_MODEL_REVIEW` (Sonnet) because the work is checklist-shaped AC verification; L/XL/delicate pick up the new `CLAUDE_MODEL_REVIEW_LARGE` env var (default Opus). Override either side to revert.

- **Worktree-canonical task state from implement onward.** Canon previously kept two copies of task state during pipeline execution — the worktree at `dev-worktrees/<id>/tasks/<id>/` and a synced mirror at `REPO_ROOT/tasks/<id>/`. The worktree is now canonical for task-scoped state from implement-phase start onward; REPO_ROOT is canonical for project-level resources and pre-implement task state. The sync machinery (`syncWorktreeArtifacts`, `syncWorktreeTelemetry`, `flushWorktreeTelemetry`, `mirrorHumanReviewDocsToCwd`) is deleted entirely. `taskDirFor` now routes through `resolveTaskCwd` so every consumer is automatically worktree-aware; pre-implement callers are unaffected. `canon task status/list/accept/phase` read from the worktree when one exists past plan, so mid-pipeline queries show live state instead of frozen scaffold values. Pre-implement dirty telemetry files are absorbed at scaffold-commit time, keeping REPO_ROOT clean once the worktree exists. Two known 1.4 failure modes resolved as side-effects: `parseAffectedFilesFromSpec` reading REPO_ROOT in worktree mode, and `--ship`'s `git pull origin main` failing because REPO_ROOT had uncommitted task-artifact modifications. PR #104.

### Added

- **`docs-refs-check` adopter skip-path surface (`NOISY_SOURCE_PATHS`).** New empty-by-default constant in `scripts/docs-refs-check.mjs` lets adopters declare paths to exclude after `canon upgrade` — exact file match (`'docs/changelogs.md'`) or directory prefix (`'docs/archive'`), trailing-slash normalized.
- **`docs-refs-check` treats `...` as a placeholder.** Backtick refs containing `...` (e.g., `` `src/...` ``) no longer produce "missing file" findings; extends the existing `<placeholder>` / `[placeholder]` handling.
- **`docs-refs-check` exempts hypothetical-friendly per-task artifacts.** `tasks/<id>/notes.md` and `tasks/<id>/spec-review.md` join the existing carve-outs for `spec.md` / `plan.md`. Both files routinely contain refs to paths the reviewer or Codex imagined but never built; treating those as broken refs forced cleanup edits unrelated to the task. `handoff.md`, `review.md`, and `done.md` remain scanned — those record real work and broken refs there are real bugs.
- **`docs-refs-check` recognizes `...` in symbol-position too.** Symbol-in-file refs like `` `<!-- canon:start -->...<!-- canon:end -->` in `AGENTS.md` `` no longer produce "symbol not found" findings when the symbol contains `...`. Narrower than the target-side placeholder predicate to avoid silencing legitimate symbol refs whose names happen to match path-oriented placeholder words (`id`, `name`, `task`).
- **`docs-refs-check` skips gitignored paths.** Refs to paths git already considers ignored (e.g., `.claude/settings.local.json`, written by Claude Code on first permission grant) no longer produce "missing file" findings. Resolves the local-vs-CI inconsistency where the check passed locally (file existed on disk) but failed on a fresh CI clone. Source markdown files that are themselves gitignored are also excluded from scanning — generated / local-only docs shouldn't be validated when they won't exist on CI. Uses one batched `git check-ignore --stdin -z` invocation; falls back to "no skip" outside a git repo so test fixtures and adopter installs remain usable.

- **`/canon-review` skill — adversarial pre-pipeline spec review.** New Claude Code skill at `.claude/skills/canon-review/SKILL.md`. Invoke as `/canon-review <task-id>` before `canon run <id>` to dispatch three parallel sub-agents (structural / factual / spec-quality) at the spec from different angles and surface BLOCKING / STRONG / NIT findings inline. Catches the class of issues Codex's spec_review would surface across 2-3 iterations in a single ~15-min pass. Spec mode only in v1; code-diff review variant is a future addition. Opt-in — recommended for M/L/XL or delicate specs where iteration cost is real.

### Fixed

- **`--pr` base-drift gate accepts directory-form Affected Files entries.** Spec rows like `` | `dist/` | … | `` now match every subpath (`dist/cli/index.js`, `dist/scripts/run-task.js`) instead of requiring exact-path enumeration. The same prefix semantics apply to the human-review dirty-tree and staging gates, so a task that declares a directory in *Affected Files* passes both checks end-to-end. Trailing slash is the boundary marker — `dist/` does not accept `dist-other/foo`.
- **`--pr` base-drift and human-review gates auto-allowlist `PIPELINE_MANAGED_DOCS` once `qa.status = done`.** QA's "Docs Freshness" sweep promotes lessons into managed docs (`docs/codebase-map.md`, `docs/patterns.md`, etc.) downstream of what the spec author could predict at spec time. The gates previously fired on any managed doc not enumerated in *Affected Files*, forcing operators to backfill `spec.md` before `--pr`. Both gates now union the full `PIPELINE_MANAGED_DOCS` set into their allowlist as soon as the task's `qa.status` reaches `done`; pre-QA edits still require explicit spec listing.
- **Orchestrator survives SIGHUP from a dying supervising shell.** When `canon run <id>` is backgrounded inside a Claude Code bash tool and the supervising shell goes away (terminal close, conversation-resume cycle, remote-shell disconnect), SIGHUP propagated down the process group and killed the Node orchestrator silently — the pipeline appeared alive in `status.json` but no process was running, and the orchestrator's in-process stall timer died with it. The fix installs `process.on('SIGHUP', () => { ... })` to log-and-ignore in `scripts/run-task.ts`, and switches agent-child stdin from `'inherit'` to `'ignore'` in `streamProcess` so children don't share the supervising shell's controlling terminal. Detach mode (PID file + `--detach` flag) and heartbeat-file detection remain open as follow-ons. PR #105.
- **Code-review pre-flight exempts pipeline telemetry files from the diff coverage check.** The pre-flight `git diff <baseRef>...HEAD --name-status` enumeration would flag `docs/pipeline-invocations.md`, `docs/task-quality-log.md`, and `docs/lessons-learned.md` rows as "uncovered diff files" if the task scaffold's absorption commit picked up dirty telemetry from a prior invocation. Those files are pipeline-owned, not implementation surface, and absorbing them into the scaffold is intentional behavior post-worktree-canonical. The fix unions `PIPELINE_TELEMETRY_FILES` into the same exemption set that already covers `tasks/<id>/` artifacts. PR #106.
...
- **`canon task release-init` inserts the new CHANGELOG block in the right place.** Previously inserted directly after the H1, pushing any intro blockquote (e.g., `> Format follows Keep a Changelog...`) below the new unreleased entry. Now anchors on the first `## [X.Y.Z]` bracketed-version block and inserts immediately before it, so file-level meta stays between the H1 and the version entries where it belongs. Initial-scaffold CHANGELOGs (no prior version block) append at the end as before.
- **Code-review pre-flight rejection no longer skips Stage 1 on subsequent rounds.** When a malformed `handoff.md` triggered the orchestrator's pre-flight gate, the rejection path bumped `iterations_current_loop` and overwrote `review.md` with a BLOCKED stub. The next code_review run saw `maxIter > 0`, was issued the round-N prompt ("Stage 1 AC table already passed in round 1 — don't redo it"), and approved without ever running Stage 1. Archive sweep found eight tasks shipped under this pattern. Fix: new `taskPhasePreflightRejected` helper records the rejection (and a new `preflight_rejections_current_loop` counter) without bumping the iteration counter, and the pre-flight path appends a `## Pre-Flight Rejection — ...` section to `review.md` instead of stomping it.
- **Round-N code_review prompt re-fills the Stage 1 AC table every round.** The prior round-N prompt told Claude "Do NOT redo the Stage 1 AC table — that gate already passed in round 1." That premise is wrong when Round 1 returned `changes_requested` (the AC table found violations, the gate didn't pass). Two failure modes the old prompt allowed: (1) multi-faceted ACs where Round 1's finding cited one violation but the AC also required something else — Codex fixed the cited violation, round-N trusted the finding, and the other violation persisted; (2) regressions where Codex's iteration accidentally broke a previously-Met AC and round-N had no systematic check. Round-N now mandates re-filling the full Stage 1 AC table, with an explicit "Met (unchanged from round N-1)" shortcut for ACs whose code paths weren't touched.
- **`canon task list` no longer crashes on orphan- or stale-worktree state.** Post worktree-canonical-task-state (PR #104), `taskList` resolves each entry through `taskDirForCwd` → `resolveTaskCwd`, which `die()`s when a task's `status.json` claims `worktree: true` with a branch but no usable worktree exists — either `dev-worktrees/<id>/` is missing entirely, OR the directory is present but stale (no `tasks/<id>/status.json` inside, as happens after a half-completed `git worktree remove`). The existing `try { … } catch { push INVALID row }` block (added for issue #83 so one malformed task wouldn't abort the listing) couldn't catch the crash because `die()` is `process.exit`, not a throwable. A new `isOrphanedWorktreeState` helper in `scripts/run-task/state.ts` mirrors `resolveTaskCwd`'s usability test (checks the nested `tasks/<id>/status.json`, not just the worktree dir) ahead of `resolveTaskCwd`; `taskList` renders an INVALID row pointing at the missing/stale `dev-worktrees/<id>/` and continues. Restores the issue #83 graceful-degradation contract.
...
> Tracked failure modes against 1.4.0. All five resolved in 1.5.0; entries kept (struck through) for traceability.

- ~~**`canon run --ship` crashes on non-worktree tasks (`ENOENT`)**~~ — resolved in 1.5.0 (PR #103).
- ~~**`parseAffectedFilesFromSpec` reads REPO_ROOT in worktree mode**~~ — resolved in 1.5.0 as a side-effect of `worktree-canonical-task-state` (PR #104).
- ~~**`--pr` base-drift gate fires on QA-promoted managed-doc edits**~~ — resolved in 1.5.0 (auto-allowlist of `PIPELINE_MANAGED_DOCS` once `qa.status = done`).
- ~~**`--pr` base-drift gate: directory-form *Affected Files* entries don't match per-file diff paths**~~ — resolved in 1.5.0 (trailing-slash entries are now treated as directory prefixes in both base-drift and human-review gates).
- ~~**Main-repo accumulates stale `tasks/<id>/` artifacts that block `git pull` during `--ship`**~~ — resolved in 1.5.0 by `worktree-canonical-task-state` (PR #104).
