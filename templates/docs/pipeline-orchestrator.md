...
| `reset-code-review` | `<id>` | Clear router-relevant state for a fresh code-review pass after an auto-block. Zeroes the loop-local review counters, clears verdict, archives the prior `review.md` and re-scaffolds a fresh one from the task templates, and drops the stored `sessions.claude_review` ID so the next pass runs against a fresh Claude review session. |
...
Codex is tuned for token efficiency — the mini model handles most phases; the full model only comes out for XL or delicate work. XL/delicate implement runs at `high`, not `xhigh`: reasoning models have shown overthinking at `xhigh` with open-ended tool access — cost and latency without a quality gain. Raise via env only if your own evaluation shows under-reasoning.
...
**Shared dependency links**: Worktree creation links the root `node_modules` and each eligible npm workspace's installed `node_modules` from `REPO_ROOT` into the task worktree. Eligible workspaces come from non-negated `workspaces` patterns in `REPO_ROOT/package.json` and must resolve inside both checkouts. A missing or hoisted workspace install is skipped silently; an absent worktree workspace is skipped with an info message; and an unsafe destination is skipped with a warning. A real file or directory at the workspace `node_modules` path is left untouched, while a wrong-target symlink or an uninspectable entry aborts initial setup. On later implement entries, canon repairs only missing workspace links in its registered task worktree; it leaves existing root, workspace, and `.env*` entries untouched.
...
When `qa.status` reaches `done`, the orchestrator commits QA artifacts before stopping at `human_review`. The QA-end commit stages the bundled `tasks/<id>/` directories plus any dirty `PIPELINE_TELEMETRY_FILES` or `PIPELINE_MANAGED_DOCS` in the active checkout, with subject `chore: QA artifacts for <task-id>` (bundles list every task id). This commit does not push or create a PR. It leaves the worktree clean for `--reroute`, avoids the `--pr` rebase stash/pop dance, and makes post-QA task state durable against local history surgery. The residual implement-to-first-QA window remains: code_review iterations before the first successful QA still have uncommitted phase artifacts. A top-level `node_modules` entry, or the canonical worktree-relative `<destination>/node_modules` for an eligible npm workspace selected by a non-negated pattern in `REPO_ROOT/package.json`, containing a regular `package.json`, not nested under `node_modules`, and contained in both `REPO_ROOT` and the active worktree, is exempt from both this gate's dirty-tree check and the `human_review` dirty-tree check below when a filesystem probe confirms it is canon's untracked symlink to the corresponding `REPO_ROOT` path; anything else at those paths (a staged entry, a real file/directory, a symlink pointing elsewhere, or an entry that fails containment) still blocks normally. For an ordinary workspace directory, `<destination>` equals the declared workspace path; for an in-repo symlinked workspace, it is that directory's canonical destination. The workspace exemption applies only while a distinct task worktree is active. After verified untracked links are removed from classification, any remaining dirty path whose final segment is exactly `node_modules` is rejected before staging.
...
Non-managed Affected Files entries (source files, test files, fixtures) do not enter the `human_review` allow-list as individual files — those belong to the implement phase's auto-commit. Two carve-outs do apply at `human_review`: `PIPELINE_MANAGED_DOCS` entries listed in Affected Files, and **directory-form entries** (a path ending in `/`, e.g. `dist/`) — a matching prefix is staged as one pathspec, which is the mechanism for build-generated artifacts regenerated late in the pipeline. One shared classifier decodes and normalizes porcelain paths for both the allow-list and stage-path selection. A non-exempt path whose final segment is exactly `node_modules` aborts before staging, while vendored descendants such as `dist/lambda/node_modules/pkg/index.js` remain eligible. Scoped exclusion pathspecs are added only for porcelain-visible links already verified as exempt, so canon's links cannot ride the prefix into a commit and ignored real workspace installs are not explicitly passed to `git add`.
...
**`Fail – unrelated` result state**: When a required check fails due to a pre-existing flake or a test outside the task's Affected Files, Codex may record `Fail – unrelated` in the Validation Outcomes table instead of blocking on a bare `Fail`. The deterministic pre-flight gate accepts this state — with one anti-laundering exception: if the Notes cite a file this task changed, the row is a regression (a failure in your own diff can't be relabeled unrelated). The gate does **not** otherwise judge the credibility of the "unrelated" claim; the code-review prompt instructs Claude to assess whether the explanation is credible and the failure is genuinely out of scope. This is part of the gate's deliberate division of labor — see the note below.

**The validation-outcome pre-flight is structural, not spec-matching.** The deterministic gate that runs before code review asserts only what it can judge unambiguously from the handoff's Validation Outcomes table: the spec declares at least one required check, every recorded row is filled in (no leftover template placeholder), and no row is an unexplained failure (a plain `Fail`, or a `Fail – unrelated` citing a changed file). It deliberately does **not** pair each spec-required check to a handoff row by prose — that string matching repeatedly false-blocked valid work when the spec's wording and the handoff's cell text drifted apart. Whether each specific required check was actually run (present, and not wrongly skipped or marked `N/A`/`not_configured`) is judged by Claude in Stage 1 code review, which reads the outcomes table against the spec — see "Where validation happens" above.
...
Reroute archives each named task's `review.md` to `review-prior-<n>.md` in the active task directory, where `<n>` is one greater than the highest archive number already present (or 1 when none exist). It archives every task before changing any task's `status.json`; if an archive fails, reroute aborts without mutating any member's status, while any already-completed rename remains safely preserved on disk. A `review.md` that is still the pristine scaffold is left in place and produces no archive. After archiving a filled review, reroute writes a fresh `review.md` scaffold into the task directory — from the project's `review.md` override under `tasks/_templates/` if it keeps one, otherwise from `.canon/templates/review.md` — so the code-review foreman fills the same template structure a new task starts with. If neither template exists, reroute warns and continues; the foreman then writes the file from scratch. Reroute also drops the stored `sessions.claude_review` ID, so post-reroute code review starts a genuine round 1 rather than resuming a session that remembers prior rounds. Archived findings remain binding; exempt-sibling prompts point at the archived file instead of the now-absent `review.md`.
