...
9. Foreman writes `review.md` and updates `status.json` with `approved`, `approved_with_nits`, `changes_requested`, `needs_re_review`, or `spec_gap`. A `sanctioned` verdict is operator-written only via `canon task accept`.
