...
### Any status write inside `reportOrCreatePR()` needs its own commit+push — "clean final state" means no dirty tree, not zero new commits

*(2026-06-07, source: ship-merge-proof)*

`commitHumanReviewFiles()` in `scripts/run-task/main.ts` commits and pushes task artifacts *before* it calls `reportOrCreatePR()`. Any new `status.json` write added inside `reportOrCreatePR()` (or any hook that runs after that commit chain) therefore happens after the push — it must trigger its own `stage → commit → push` cycle or it leaves a dirty, unpushed state that silently corrupts subsequent gates. This ordering constraint is not obvious from the call site: `reportOrCreatePR()` looks like a reporting step, not a write step. The corollary for spec-writing: an AC asserting that `--pr` exits "cleanly" should assert an empty `git status --porcelain` tree (no dirty tree), not "zero commits since the test started" — a legitimate follow-up commit (e.g., canon snapshot refresh recording the new HEAD) can land after the pin commit and still satisfy clean-exit. The tell: a `--pr` run that exits without error but leaves a modified, unpushed `status.json` in the tree.
