...
**No existing `CHANGELOG.md` format to match**: if `docs/decisions.md §"Versioning and release policy"` specifies a format, use that. Otherwise propose Keep a Changelog as the starting point. Surface the proposed default to the human and ask for confirmation before creating or rewriting the file.
