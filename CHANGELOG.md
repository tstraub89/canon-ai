...
- **Shipped agent guidance no longer assumes the adopter is a consumer app with paid tiers.** The context-loading instructions injected into every Claude and Codex session told agents to read `docs/product-context.md` "if the task touches user-visible behavior or **Pro features**" and to skip `docs/decisions.md` "unless the task involves explicit **UX tradeoffs**" — a "Pro" tier and a UX surface are artifacts of the project canon was extracted from, not properties of an adopter's repo. canon-ai itself fails all of those predicates, so they were dead weight even in dogfooding. Each doc now gets its own predicate stated in terms of that doc's own documented scope: `docs/architecture.md` on first-time orientation or a change to the tech stack, a load-bearing dependency, core data flow, or an architectural boundary; `docs/product-context.md` on user-visible behavior, product terminology, or a business rule; `docs/decisions.md` when a settled decision governs the area being changed, not only when the task would revisit one.
...
- **`canon doctor` no longer points adopters at paths and docs that exist only in canon-ai's repo.** A malformed `docs/task-quality-log.md` warned "compare with `templates/docs/task-quality-log.md`" — present in canon-ai's own checkout, in no adopter repo, and with no vendored `.canon/templates/` counterpart to fall back on. The warning now states the requirement directly from `CANON_LOG_HEADERS` (every header cell unique, and the full required column set present), so it cannot drift from the check and needs no reference file; it also now describes the duplicate-header failure mode, which the previous wording left out even though `locateLogTable` rejects it. Separately, three permission-check warnings said "see README", meaning canon-ai's README rather than the adopter's own; they now name it as the canon-ai README.
