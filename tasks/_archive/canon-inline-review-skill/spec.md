...
**Reroute 1 — simplify the target-selection contract (AC-3): intent from context decides, not git-state inference.**

Three review rounds (spec/code/PR) each surfaced the same bug class: auto-detecting the review target from git state has an irreducible edge case (default-on-`--uncommitted`, then the ahead-check, then upstream-vs-base). Per canon's iteration-vs-design lesson, same class round-over-round = the design is wrong. The standard pattern for running codex headless from a skill ([amanhimself.dev "Running headless Codex CLI inside Claude Code"](https://amanhimself.dev/blog/running-headless-codex-cli-inside-claude-code/); Claude Code headless docs) gathers ambiguous input from context / `AskUserQuestion`, not git plumbing.

Supersedes AC-3's prior procedure (a)–(d):
- **Intent from context decides the target** — operator request + conversation + `$ARGUMENTS`; the skill reasons about what to review, it does not derive intent from git plumbing.
- **Remove all git-state intent inference** — no ahead-check, no `git log @{u}..HEAD`, no `git log <base>..HEAD` committed-work detection.
- **Keep one git check as a no-op guard only**: a single `git status --porcelain`; if the resolved target is `--uncommitted` and the tree is clean, don't run an empty review — reconcile from context or ask.
- **Genuine ambiguity → `AskUserQuestion`**.
- **Harden the invocation**: add `--sandbox read-only` (the review only reads); `--ephemeral` optional.
- Unchanged: scope bound (AC-4), `codex review` = `codex exec review` shorthand, selector-XOR-prompt, the gotchas (single-commit diff, no multi-SHA, `--uncommitted` excludes committed work), `codex exec review --help` as the flag source of truth.
...
- [ ] AC-2: When invoked, the skill drives an actual cross-review — it selects the target from operator intent/context, runs `codex review` / `codex exec review` non-interactively, and surfaces the findings to the session (a concise summary, not a raw dump). Verify: Human Test Plan steps 1–3 (run on a real dirty tree → it runs the review and reports findings).
- [ ] AC-3: The skill selects the review target **from operator intent/context, not git-state inference**, and treats `codex exec review --help` as the version-pinned source of truth for flags. The contract: (a) determine intent from the operator's request + conversation + `$ARGUMENTS` and map it — commit / "last commit" / SHA → `--commit <SHA>` (resolve via read-only `git rev-parse` / `git log`); branch / "whole branch" / pre-PR → `--base <branch>`; steering text → positional `PROMPT`; explicit "uncommitted" / "before commit" / no other signal → `--uncommitted`; (b) **no git-state inference of intent** — no ahead-check, no `git log <ref>..HEAD` committed-work detection; (c) **one git check, as a no-op guard only**: a single `git status --porcelain`; if the resolved target is `--uncommitted` and the tree is clean, do not run an empty review — reconcile from context or ask; (d) **genuine ambiguity → `AskUserQuestion`**, not a guess; (e) invoke with **`--sandbox read-only`** (the review only reads). Custom steering uses the positional `PROMPT`, which cannot be combined with a target selector (selector XOR prompt). Verify: SKILL.md states (a)–(e) and the mutual exclusivity; **no `git log`-based committed-work detection remains** (grep); flag names match `codex exec review --help`; it notes `codex review` is the shorthand for `codex exec review`.
...
| File | Change |
|---|---|
| `.claude/skills/canon-inline-review/SKILL.md` | **CREATE** — new workflow skill: frontmatter; when-to-use guardrail; the AC-3 target-selection contract (intent-from-context → `--uncommitted` / `--commit` / `--base` / `PROMPT`; one no-op `git status` guard, no ahead-detection; `AskUserQuestion` on ambiguity); runs `codex review` with `--sandbox read-only`; reports findings concisely; scope bound; flag reference pointing at `codex exec review --help`. |
