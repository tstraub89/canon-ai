...
- [ ] **AC-3**: `package.json` `scripts` does NOT include a `run-task` entry.
...
- [ ] **AC-15**: The `release-init` subcommand remains idempotent. Running it twice in succession produces no destructive change on the second run (matches existing `cmd_release_init` behavior). A test verifies this. Tests must NOT execute real `git push` — the push call site is mocked or stubbed.
...
- [ ] **AC-25**: The CI step "Verify git-install path produces a working canon binary" passes. Specifically: a fresh `npm install -g "git+file://$WORKSPACE"` in a clean tmpdir followed by `canon --version`, `canon doctor`, and `canon task new test-task "Test"` all exit 0. (The `task new` invocation must succeed end-to-end against a temp project — exercises the ported `cmd_new` logic in-process.) If the existing CI step does not yet invoke `canon task new`, extend it to do so as part of this AC.
...
| `package.json` | `files`: drop `"scripts/"` and `"public/"` (final value: `["dist/", "templates/"]`). `dependencies`: remove `tsx`. `devDependencies`: ensure `tsx` is present (add if not). `scripts`: remove the `run-task` entry. |
...
- [x] Local install smoke (Codex during implement): `npm pack && extract && node package/dist/cli/index.js --version` and `... task new smoke "Smoke"` in a temp dir → must succeed
- [ ] E2E — N/A (no UI)
...
- **`release-init` mutates branch state.** Existing `cmd_release_init` (lines ~567-684) creates/switches branches, pushes to remote, and is documented as idempotent. The TS port must preserve idempotency AND tests must NOT execute real `git push` — the push call site is mocked or stubbed. A regression where `release-init` re-pushes or re-creates a branch on every invocation is a silent footgun.
...
- [x] Symbols named in ACs verified to exist in the codebase: `resolveTsx`, `runTaskScript`, `taskScript`, `loadTemplate`, `TEMPLATE_DIR`, `TEMPLATE_CACHE`, `runTaskShFor`, `TASK_SH`, `resolve_task_cwd`, `cmd_new`, `cmd_list`, `cmd_status`, `cmd_phase`, `cmd_reset_spec_review`, `cmd_post_merge_sync`, `cmd_release_init`, `checkBinary`, `Bash(jq *)`, `Bash(npx tsx *)`, `resolveTaskCwd` — all confirmed via grep at spec-write time.
