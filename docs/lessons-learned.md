...
### Include docs-refs-check in Validation Required whenever the spec touches docs/, templates/, or skill markdown

*(2026-06-20, source: internal-leak-gate-and-matrix-sync)*

`docs/architecture.md` §Validation states that `npm run docs-refs-check` is required for any change touching `docs/`, `templates/`, or root-level agent files. In practice, spec authors writing the Validation Required section focus on the primary change type (linting, tests, build) and forget to include this check when the same task also edits a `docs/` file or a skill markdown file as a secondary change. The concrete case: the initial spec for this task edited `docs/decisions.md` and `.claude/skills/canon-changelog/SKILL.md` but omitted `docs-refs-check` from Validation Required; spec_review caught the gap and it was corrected before implementation. Prevention: before finalizing the Validation Required section of any spec, scan the Affected Files table for paths under `docs/`, `templates/`, or `.claude/skills/` — if any appear, add `docs-refs-check` to the required checks. The rule is in `docs/architecture.md` §Validation; this lesson is the "hot-path" reminder for spec authorship.
