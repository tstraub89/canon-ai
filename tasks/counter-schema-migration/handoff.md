...
| AC-8: Existing test suite passes unchanged. | Partial | `npm run lint` and `npm run type-check` pass; `npm test` still hits the existing runtime-validation fixture writes under `/Users/tstraub/canon-ai/canon-ai-dev/tasks/...`, which is blocked by sandbox permissions in this environment. |
...
- [unrelated] `npm test` still fails in `tests/run-task-runtime-validation.test.ts` because those fixtures try to `mkdir` under `/Users/tstraub/canon-ai/canon-ai-dev/tasks/...`, which is outside the writable sandbox. Repro: run `npm test`; the failing cases are the runtime-validation tests starting at `tests/run-task-runtime-validation.test.ts:96`.
...
| `npm test` | Fail - unrelated | Runtime-validation tests fail on sandbox permissions when they try to write fixtures under the supervising checkout path; see Blockers. |
