...
The `delicate` flag is for surfaces where a regression has **unbounded blast radius**. For canon-ai itself, this includes the orchestrator's phase-routing logic, auto-commit logic, and validation gates — a bug in any of these can corrupt every future task.
